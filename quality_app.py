...
# They are now pointing to the 'to_github' subdirectory
...
with col_predict:
    ...
    with st.form(key='prediction_form'):
        ...
        # --- بخش جدید: تعریف تارگت‌های کیفی ---
        st.markdown("---")
        st.subheader("کنترل کیفیت")
        st.markdown("**تارگت‌های کیفی مورد انتظار**")
        target_impact = st.number_input("حداقل خواص ضربه (J/m²)", min_value=0.0, key="target_impact")
        target_tensile = st.number_input("حداقل استحکام کششی (MPa)", min_value=0.0, key="target_tensile")

        predict_button = st.form_submit_button(label='پیش‌بینی و کنترل کیفیت')
        ...
        if predict_button:
            if impact_model is not None and tensile_model is not None:
                ...
                if predictions:
                    ...
                    # --- مقایسه نتایج با تارگت‌های کیفی ---
